...
cert = "/Users/pof/PycharmProjects/workfast/check_status/client.cert.pem"
key = "/Users/pof/PycharmProjects/workfast/check_status/client.key.nopwd.pem"

nas_cert = "./nas-client-cert.pem"
nas_key = "./nas-client-key.pem"
...
def login(env):
   ...
   # r = requests.post(url=url, data=json.dumps(body), verify=False)
    r = requests.post(url=url, data=json.dumps(body), cert=(nas_cert, nas_key))
    return r.json()
...
def searchby_id(env, uid, token):
    ...
    r = requests.get(url=url, headers=header, cert=(nas_cert, nas_key))
    return r.json()
